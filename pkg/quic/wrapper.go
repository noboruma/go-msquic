...
func init() {
	...
	go func() {
		for {
			...
			cores, counters := cGetPerfCounters()

			println("cores:", cores)
			for i := range counters {

				println(names[i], ":", counters[i])
			}
		}
	}()
}
...
func cGetPerfCounters() (int, []uint64) {
	counters := make([]uint64, C.QUIC_PERF_COUNTER_MAX)
	cores := C.GetPerfCounters((*C.uint64_t)(unsafe.SliceData(counters)))
	return int(cores), counters
}
