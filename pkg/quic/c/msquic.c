...
int64_t StreamWrite(_In_ HQUIC Connection,
                    _In_ HQUIC Stream,
                    _In_ uint8_t* array,
                    _In_ int64_t len,
    ...
    QUIC_STATUS Status;
    if (QUIC_FAILED(
            Status = MsQuic->StreamSend(Stream, SendBuffer, 1, QUIC_SEND_FLAG_NONE, SendBuffer))) {
        free(SendBuffer);
        return -1;
    }
    return len;
...
void AbortStream(HQUIC stream) {
    MsQuic->StreamShutdown(stream, QUIC_STREAM_SHUTDOWN_FLAG_ABORT, 0);
}

void AbortSendStream(HQUIC stream) {
    MsQuic->StreamShutdown(stream, QUIC_STREAM_SHUTDOWN_FLAG_ABORT_SEND, 0);
}

void FreeStream(HQUIC stream) { MsQuic->StreamClose(stream); }
    ...
    switch (Event->Type) {
        ...
        case QUIC_STREAM_EVENT_RECEIVE:
            ...
            if (Event->RECEIVE.BufferCount > 0) {
                uint32_t n = newReadCallback(Context, Stream, Event->RECEIVE.Buffers,
        ...
        case QUIC_STREAM_EVENT_RECEIVE_BUFFER_NEEDED:
            provideNewBuffersCallback(Context, Stream,
...
int32_t AttachAppBuffer(HQUIC Stream, QUIC_BUFFER* buffer) {
    QUIC_STATUS Status;
...
void ShutdownConnection(HQUIC connection) {
    MsQuic->ConnectionShutdown(connection, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, 0);
}

void AbortConnection(HQUIC connection) {
...
HQUIC
CreateStream(_In_ HQUIC Connection, _In_ int8_t useAppBuffers) {
    QUIC_STATUS Status;
    HQUIC Stream = NULL;
    ...
    if (QUIC_FAILED(
            Status = MsQuic->StreamOpen(Connection, flag, StreamCallback, Connection, &Stream))) {
...
int64_t StartStream(_In_ HQUIC Stream, _In_ int8_t FailOpen, _In_ int8_t useAppBuffers) {
    if (LOGS_ENABLED) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) _Function_class_(QUIC_CONNECTION_CALLBACK) QUIC_STATUS QUIC_API
    ConnectionCallback(_In_ HQUIC Connection,
    ...
    switch (Event->Type) {
        ...
        case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED:
            ...
            MsQuic->SetCallbackHandler(Event->PEER_STREAM_STARTED.Stream, (void*)StreamCallback,
                                       Connection);
            break;
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_LISTENER_CALLBACK) QUIC_STATUS QUIC_API
    ListenerCallback(_In_ HQUIC Listener,
                     _In_opt_ void* Context,
...
HQUIC
LoadListenConfiguration(_In_ struct QUICConfig cfg) {
    ...
    Settings.TlsServerMaxSendBuffer = 4 * 1024 * 1024;
...
HQUIC
CreateListener(_In_ HQUIC configuration) {
    QUIC_STATUS Status;
    HQUIC listener = NULL;

    if (QUIC_FAILED(Status = MsQuic->ListenerOpen(Registration, ListenerCallback, configuration,
                                                  &listener))) {
...
int StartListener(_In_ HQUIC listener,
...
int GetRemoteAddr(_In_ HQUIC conn, _Out_ QUIC_ADDR* addr, _Out_ uint32_t* addrLen) {
    if (MsQuic->GetParam(conn, QUIC_PARAM_CONN_REMOTE_ADDRESS, addrLen, addr) !=
        QUIC_STATUS_SUCCESS) {
